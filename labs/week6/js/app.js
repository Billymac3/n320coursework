    ...
    checkWin: function() {
      ...
      for (var row = 0; row < this.grid.length; row++) {
        for (var cell = 0; cell < this.grid[row].length; cell++) {
          if (row - 3 >= 0) {
            if (
              this.grid[row - 3][cell] == this.grid[row - 2][cell] &&
              this.grid[row - 1][cell] == this.grid[row][cell] &&
              this.grid[row][cell] == this.grid[row - 3][cell] &&
              (this.grid[row][cell] == 1 || this.grid[row][cell] == 2)
            ) {
              this.gameOver = true;
          ...
          if (cell - 3 >= 0) {
            if (
              this.grid[row][cell - 3] == this.grid[row][cell - 2] &&
              this.grid[row][cell - 1] == this.grid[row][cell] &&
              this.grid[row][cell] == this.grid[row][cell - 3] &&
              (this.grid[row][cell] == 1 || this.grid[row][cell] == 2)
            ) {
              this.gameOver = true;
              this.winningplayer = this.playerturn;
          ...
          if (row - 3 >= 0 && cell - 3 >= 0) {
            if (
              this.grid[row - 3][cell - 3] == this.grid[row - 2][cell - 2] &&
              this.grid[row - 1][cell - 1] == this.grid[row][cell] &&
              this.grid[row][cell] == this.grid[row - 3][cell - 3] &&
              (this.grid[row][cell] == 1 || this.grid[row][cell] == 2)
            ) {
              this.gameOver = true;
              this.winningplayer = this.playerturn;
